...
def runGait(individual):
    ...
    for ii in range(10000):
        setServoStatesLegs(gait.evaluate((time.time() - programStartTime) * 0.5), individual[1], 0)
        print(ii)
        # p.setRealTimeSimulation(1)
...
def evaluateGait2(individual):
    ...
    while True:
        ...
        print(f'Evaluation: {gaitEvaluation}')
        print(f'Chromosome: {individual}')
        print(f'Hexapod_ID: {hexapod_ID}')
        print(f'Distance: {distanceFromOrigin(hexapod_ID)}')
        print(f'Distance2: {p.getBasePositionAndOrientation(hexapod_ID)[0][1]}')
        # print(f'Angle: {angleBetweenVectors(hexapod_ID)}')
        if not math.isnan(gaitEvaluation[0]):
            break
        else:
            resetPyBulletSimulation()
